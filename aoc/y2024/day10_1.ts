...
class Y2024_Day10_1 extends AbstractRiddle {
    ...
    riddle: string = "What is the sum of the scores of all trailheads on your topographic map?";
    ...
    run(input: string[]): number {
        ...
        for (const hikingTrail of hikingTrails) {
            ...
            const reachableTiles: {[key: string]: Vector} = {};
            ...
            while (tilesToCheckNeighboursOf.length > 0) {
                ...
                if (!tileToCheck || reachableTiles[tileToCheck.toString()]) {
                    continue;
                }
                reachableTiles[tileToCheck.toString()] = tileToCheck;
